...
class RCDriver(object):
    ...
    def backtrack(self):
        ...
        #
        # backtrack_steps_local = backtrack_steps
        #
        # print ' < < < BACKTRACKING > > > '
        #
        # # Accounts for instance when the steering log has fewer entries than the backtrack steps to take (i.e. the car messes up very soon after it starts).
        # if len(self.steering_log) < backtrack_steps_local:
        #     backtrack_steps_local = len(self.steering_log)
        #
        # for i in reversed(self.steering_log)[ :backtrack_steps_local]:
        #
        #

        # Adjust SIGMA up or down (to let in more noise to hopefully pick up more signal)
        pass
...
class NeuralNetwork(object):
    ...
    def fetch_execute(self):
        ...
        while self.receiving:
            ...
            # # If prediction_english_proba < min_steer_proba...
            # # 'min_steer_proba' is the minimum probability a model argmax prediction must have in order to execute. If proba is less than this, then execute the backtrack maneuver.
            # if prediction_english_proba < min_steer_proba:
            #     self.rcdriver.backtrack()

            # If the image hasn't changed since the last image, then you can assume the car is stuck somewhere.


            # Send prediction to driver to tell it how to steer
            self.rcdriver.steer(prediction)

            # Append direction to steering log
            self.rcdriver.steering_log.append(prediction_english)
...
if __name__ == '__main__':
    try:
        ...
        os.rename(  './test_frames_temp', './test_frames_SAVED/test_frames_{}_{}'.format(float(SIGMA)[-2:], timestr))
        os.makedirs('./test_frames_temp')

        print ' \"HALT!\" '
        print '! Received keyboard interrupt, closing socket and threads...'
        car.pause(10000)
